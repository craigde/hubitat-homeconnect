...
metadata {
    definition(name: "Home Connect CleaningRobot", namespace: "craigde", author: "Craig Dewar") {
        capability "Sensor"
        capability "Switch"
        ...
        // Cleaning Robot specific events
...
private void handleHomeConnectEvent(Map item) {
    ...
    switch(key) {
        ...
        case "ConsumerProducts.CleaningRobot.Event.EmptyDustBoxAndCleanFilter":
            sendEvent(name: "EmptyDustBoxAndCleanFilter", value: value?.substring(value?.lastIndexOf(".") + 1), isStateChange: true)
            break
        case "ConsumerProducts.CleaningRobot.Event.RobotIsStuck":
            sendEvent(name: "RobotIsStuck", value: value?.substring(value?.lastIndexOf(".") + 1), isStateChange: true)
            break
        case "ConsumerProducts.CleaningRobot.Event.DockingStationNotFound":
            sendEvent(name: "DockingStationNotFound", value: value?.substring(value?.lastIndexOf(".") + 1), isStateChange: true)
            break
