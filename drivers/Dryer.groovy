 ...
 *
 *  Home Connect Dryer (Child Device of Home Connection Integration)
 *
...
metadata {
    definition(name: "Home Connect Dryer", namespace: "craigde", author: "Craig Dewar") {
        capability "Sensor"
        capability "Switch"
...
private void handleHomeConnectEvent(Map item) {
    ...
    switch(key) {
        ...
        // ============ Dryer specific options ============
        case "LaundryCare.Dryer.Option.DryingTarget":
            String dryTarget = value?.tokenize('.')?.last() ?: displayValue
            sendEvent(name: "DryingTarget", value: dryTarget, isStateChange: true)
            break
